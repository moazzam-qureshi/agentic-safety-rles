...
| 5 | "A Dockerized environment with all fixtures" | DONE | `docker/`, two images |
...
| "Docker familiarity for environment setup" | `docker/Dockerfile.agent`, `docker/Dockerfile.grader` |
...
| Requirement | Status |
|---|---|
| "task, setup, and grader in one reproducible package (JSON/JSONL, Docker/Git)" | DONE — git + pinned images |
...
All six per-package deliverables and the full 1 + 4 scope are complete.
Every package passes its branch matrix both locally and inside the container.

Remaining: make the repository public, and write the proposal.
